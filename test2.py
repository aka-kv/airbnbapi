...
def calculate_occupancy_rate(data):
    today = datetime.now()
    five_months_later = today + timedelta(days=5*30)  
    ...
    if total_days_count > 0:
        ...
        availability_rate = 0
    print(f"Availability Rate for the Next 5 Months: {availability_rate:.2f}%")
def main():
    listing_id = input("Enter the Airbnb listing ID: ")
    occupancy_data = fetch_airbnb_occupancy_data(listing_id)
    if occupancy_data:
        calculate_occupancy_rate(occupancy_data)
    ...
    if valid_prices:
        ...
        average_price = calculate_average_price(valid_prices)
        print(f"Highest Price: ₹{highest_price}")
        print(f"Lowest Price: ₹{lowest_price}")
        print(f"Average Nightly Price: ₹{average_price:.2f}")
    else:
        print("Highest Price: not available")
        print("Lowest Price: not available")
        print("Average Nightly Price: not available")
